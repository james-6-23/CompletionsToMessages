//! 格式转换模块
//!
//! 实现 Anthropic ↔ OpenAI 格式转换
//! 来源: cc-switch 项目 (src-tauri/src/proxy/providers/transform.rs)
...
pub fn anthropic_to_openai(body: Value, cache_key: Option<&str>) -> Result<Value, ProxyError> {
    ...
    // NOTE: 模型映射由上游统一处理（model_mapper），格式转换层只做结构转换。
...
mod tests {
    ...
    fn test_openai_to_anthropic_with_legacy_function_call() {
        let input = json!({
            ...
            "choices": [{
                "index": 0,
                "message": {"role": "assistant", "content": null, "function_call": {"name": "get_weather", "arguments": "{\"location\":\"Tokyo\"}"}},
